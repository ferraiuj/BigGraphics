...
public class Spritesheet {
	...
	public Spritesheet(Spritesheet sheet, int xPosition, int yPosition, int width, int height, int spriteSize) {
		int xx = xPosition * spriteSize;
		int yy = yPosition * spriteSize;
		int spritePointerWidth = width * spriteSize;
		int spritePointerHeight = height * spriteSize;

		if (width == height)
			SIZE = width;
		else
			SIZE = -1;
		SPRITE_WIDTH = spritePointerWidth;
		SPRITE_HEIGHT = spritePointerHeight;

		pixels = new int[spritePointerWidth * spritePointerHeight];
		for (int y0 = 0; y0 < spritePointerHeight; y0++) {
			int yp = yy + y0;
			for (int x0 = 0; x0 < spritePointerWidth; x0++) {
				int xp = xx + x0;
				pixels[x0 + y0 * spritePointerWidth] = sheet.pixels[xp + yp * sheet.SPRITE_WIDTH];
